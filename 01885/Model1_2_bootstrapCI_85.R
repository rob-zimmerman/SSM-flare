...
pars.hat <- c(0.98500070, 0.12151959, 0.19774530, 0.08052196)
...
llk.M1 <- function(phi.1.t, lsigma.1, lbeta.1, lbeta.2, Y)
{
  phi.1 <- tanh(phi.1.t)
...
gen_data_and_est <- function(phi1, sig1, bet1, bet2) {
  ...
  for (t in 1:TT) {
    Y.r[t,] <- c(rpois(n=1, lambda=w*bet1*exp(X1.r[t])), rpois(n=1, lambda=w*bet2*exp(X1.r[t])))
  }
  
  pars.r <- optim(par=c(2.20, -2.14, 1.68, -2.57),
...
BS <- foreach(b = 1:B, .errorhandling = "pass", .options.RNG = 1729) %dopar% {
  ...
  MLE.b <- gen_data_and_est(phi.1.hat, sigma.1.hat, beta.1.hat, beta.2.hat)
...
save.image("Model1_2_bootstrapCI_85.Rdata")

bias.BS <- colMeans(t(sapply(BS, function(x) partrans.M1(x$par)))) - pars.hat # bootstrap estimate of bias
sd.BS <- sqrt(diag(cov(t(sapply(BS, function(x) partrans.M1(x$par)))))) # bootstrap estimate of SEs

pars.hat.cor <- pars.hat - bias.BS # bias-corrected MLE
CI.L.cor <- pars.hat.cor - 1.96*sd.BS
CI.U.cor <- pars.hat.cor + 1.96*sd.BS

tab_nums.M1 <- data.frame(Estimate = pars.hat.cor,
                          Std_Err = sd.BS,
                          CI.L = CI.L.cor,
                          CI.U = CI.U.cor)

colnames(tab_nums.M1) <- c("Estimate",
                           "Standard Error",
                           "CI (Lower)",
                           "CI (Upper)")
